    ...
    with engine.begin() as conn:
        for _, row in relations_df.iterrows():
            ...
            try:
                ...
                # Use TRIM() for checking for empty source values
                initially_null_conditions = " OR ".join([
                    f"({col} IS NULL OR TRIM(CAST({col} AS VARCHAR(MAX))) = '')" for col in source_columns
                ])
                ...
                # UPDATED: Use LOWER and TRIM on both sides of the join condition
                join_conditions = " AND ".join([
                    f'LOWER(TRIM(CAST(src."{sc}" AS VARCHAR(MAX)))) = LOWER(TRIM(CAST(ref."{lc}" AS VARCHAR(MAX))))'
                ...
                # Use TRIM() for checking for non-empty but unmatched values
                unmatched_check_conditions = " AND ".join([
                    f"({col} IS NOT NULL AND TRIM(CAST({col} AS VARCHAR(MAX))) <> '')" for col in source_columns
                ])
                unmatched_query = text(f"""
...
def implement_many_to_many_relations(target_db_url: str, schema_name: str, csv_path: str = "config/data_integrity_changes/many_to_many_relations.csv"):
    ...
    for _, row in relations_df.iterrows():
        ...
        try:
            ...
            # UPDATED: Clean lookup keys to be lowercase and stripped of whitespace
            for col in lookup_name_cols:
                lookup_df[col] = lookup_df[col].astype(str).str.lower().str.strip()
            ...
            for source_chunk_df in pd.read_sql(source_sql, engine, chunksize=PROCESSING_CHUNK_SIZE):
                ...
                # UPDATED: Clean source keys to be lowercase and stripped of whitespace
                for col in source_multi_cols:
                    exploded_df[col] = exploded_df[col].astype(str).str.lower().str.strip()
